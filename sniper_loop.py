...
def analisar_sinal(iq, par_base):
    ...
    try:
        ...
        # EMAs sobre velas FECHADAS
        e7  = ema(closes[-20:], 7)
        e9  = ema(closes[-20:], 9)
        e21 = ema(closes[-21:], 21)

        # Direção
        call_pts = sum([e7 > e9, e9 > e21, c_atual > e9])
        put_pts  = sum([e7 < e9, e9 < e21, c_atual < e9])
        ...
        if call_pts >= 2 and call_pts > put_pts:
            ...
            return None, 0, f'EMAs sem consenso (CALL={call_pts} PUT={put_pts})'

        # RSI — bloqueia exaustão extrema
        rsi = calcular_rsi(closes)
        if direction == 'CALL' and rsi > 72:
            return None, 0, f'RSI sobrecomprado ({rsi:.1f}) — exaustão CALL'
        if direction == 'PUT' and rsi < 28:
            return None, 0, f'RSI sobrevendido ({rsi:.1f}) — exaustão PUT'
        ...
        # ── SCORE — critérios-âncora obrigatórios ───────────────────
        score       = 0
        ancoras_ok  = 0  # deve ter pelo menos 2 dos 3 critérios principais

        # ÂNCORA 1: EMAs alinhadas (60 pts)
        ema_ok = False
        if direction == 'CALL':
            if e7 > e9:  score += 20
            if e9 > e21: score += 20
            if c_atual > e21: score += 20
            ema_ok = (e7 > e9 and e9 > e21)
        else:
            if e7 < e9:  score += 20
            if e9 < e21: score += 20
            if c_atual < e21: score += 20
        ...
        # ÂNCORA 2: RSI zona ideal (30 pts)
        rsi_ok = False
        if 40 <= rsi <= 60:
            score  += 30
            rsi_ok  = True
            ancoras_ok += 1
        elif direction == 'CALL' and 30 <= rsi < 40:
            score += 15
        elif direction == 'PUT' and 60 < rsi <= 70:
            score += 15
...
def rodar_ciclo(iq, estado):
    ...
    try:
        ...
        # D. Confirmação rápida: EMAs não inverteram
        velas_conf = iq.get_candles(nome_par, 60, 25, time.time())
        if velas_conf and len(velas_conf) >= 20:
            # Exclui a vela atual (aberta) da confirmação também
            conf_fechadas = velas_conf[:-1]
            closes_c = [c['close'] for c in conf_fechadas]
            e7c = ema(closes_c[-20:], 7)
            e9c = ema(closes_c[-20:], 9)
            if direction == 'CALL' and e7c < e9c:
                log('Confirmação: EMAs inverteram contra CALL — abortando.')
                return None
            if direction == 'PUT' and e7c > e9c:
                log('Confirmação: EMAs inverteram contra PUT — abortando.')
                return None
